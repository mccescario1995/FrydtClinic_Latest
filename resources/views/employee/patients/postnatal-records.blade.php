...
<div style="max-width: 1200px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <div>
            <h1 style="color: #333; margin: 0; font-size: 28px;">Postnatal Records</h1>
            <p style="color: #666; margin: 5px 0 0 0;">Patient: {{ $patient->name }}</p>
        </div>
        <div style="display: flex; gap: 10px;">
            <a href="{{ route('employee.patients.create-postnatal-record', $patient->id) }}" style="background: #28a745; color: white; text-decoration: none; padding: 10px 20px; border-radius: 5px; font-size: 14px;">+ Add Postnatal Record</a>
            <a href="{{ route('employee.patients.medical-records', $patient->id) }}" onclick="sessionStorage.setItem('lastMedicalRecordsTab', 'postnatal');" style="background: #6c757d; color: white; text-decoration: none; padding: 10px 20px; border-radius: 5px; font-size: 14px;">← Back</a>
        </div>
    </div>
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background: #f8f9fa; border-bottom: 2px solid #dee2e6;">
                            ...
                            <th style="padding: 15px; text-align: left; font-weight: bold; color: #333; border-right: 1px solid #dee2e6;">Days Postpartum</th>
                            ...
                            <tr style="border-bottom: 1px solid #dee2e6;">
                                ...
                                <td style="padding: 15px;">
                                    <div style="display: flex; gap: 8px;">
                                        <a href="{{ route('employee.patients.show-postnatal-record', [$patient->id, $record->id]) }}" style="background: #17a2b8; color: white; text-decoration: none; padding: 6px 12px; border-radius: 3px; font-size: 12px; display: inline-block;">View</a>
                                        <form method="POST" action="{{ route('employee.patients.delete-postnatal-record', [$patient->id, $record->id]) }}" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this postnatal record?')">
                                            @csrf
                                            @method('DELETE')
    ...
    @else
        ...
        <div style="background: white; border: 2px dashed #dee2e6; border-radius: 8px; padding: 60px 20px; text-align: center;">
            ...
            <h3 style="color: #666; margin: 0 0 10px 0;">No Postnatal Records Found</h3>
            <p style="color: #999; margin: 0 0 20px 0;">There are no postnatal records for this patient yet.</p>
            <a href="{{ route('employee.patients.create-postnatal-record', $patient->id) }}" style="background: #28a745; color: white; text-decoration: none; padding: 12px 24px; border-radius: 5px; font-size: 14px; display: inline-block;">
                Add First Postnatal Record
            </a>
        </div>
    @endif
</div>
