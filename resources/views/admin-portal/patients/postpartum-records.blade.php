...
<div class="page-header">
    <h1 class="page-title">
        <i class="fas fa-female me-2"></i>Postpartum Records
    </h1>
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div>
        <a href="{{ route('admin-portal.patients.create-postpartum-record', $patient->id) }}" class="btn btn-admin-primary">
            <i class="fas fa-plus me-1"></i>Add Postpartum Record
        </a>
    </div>
</div>
    ...
    <div class="card-body">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($record->postpartum_depression_screening)
                                        <span class="badge bg-warning">Screening Done</span>
                                    @else
                                        <span class="badge bg-secondary">Not screened</span>
                                    @endif
                                </td>
                                <td>
                                    <div class="btn-group" role="group">
                                        ...
                                        <form method="POST" action="{{ route('admin-portal.patients.delete-postpartum-record', [$patient->id, $record->id]) }}" class="d-inline"
                                              onsubmit="return confirm('Are you sure you want to delete this postpartum record? This action cannot be undone.')">
                                            @csrf
                                            @method('DELETE')
        ...
        @else
            <div class="empty-state">
                ...
                <h5>No Postpartum Records</h5>
                <p class="text-muted">This patient has no postpartum records yet.</p>
                <a href="{{ route('admin-portal.patients.create-postpartum-record', $patient->id) }}" class="btn btn-primary">
                    <i class="fas fa-plus me-1"></i>Add First Postpartum Record
                </a>
            </div>
        @endif
    </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Postpartum Record Details</h5>
...
function showRecordDetails(recordId) {
    fetch(`/admin-portal/patients/${{{ $patient->id }}}/postpartum-records/${recordId}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                let content = `
                    ...
                    <div class="row mt-3">
                        <div class="col-md-6">
                            <h6>Mental Health & Breastfeeding</h6>
                            <p><strong>Mood Assessment:</strong> ${record.mood_assessment || 'N/A'}</p>
                            <p><strong>Depression Screening:</strong> ${record.postpartum_depression_screening ? 'Completed' : 'Not done'}</p>
                            <p><strong>Breastfeeding Status:</strong> ${record.breastfeeding_status || 'N/A'}</p>
                        </div>
                        <div class="col-md-6">
                            <h6>Family Planning</h6>
                            <p><strong>Contraceptive Method:</strong> ${record.contraceptive_method || 'N/A'}</p>
                            <p><strong>Next Contraceptive Visit:</strong> ${record.next_contraceptive_visit ? new Date(record.next_contraceptive_visit).toLocaleDateString() : 'N/A'}</p>
