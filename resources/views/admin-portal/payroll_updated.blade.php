                            ...
                            @foreach($payrolls as $payroll)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($payroll->getDeductionSummary())
                                            <br><small class="text-muted">
                                                <a href="#" data-bs-toggle="tooltip" title="
                                                    @foreach($payroll->getDeductionSummary() as $deduction)
                                                        {{ $deduction['name'] }}: ₱{{ number_format($deduction['amount'], 2) }}
                                                        @if(!$loop->last)
                                                            {{ "\n" }}
                                                        @endif
                                                    @endforeach
            ...
            <form action="{{ route('admin-portal.payroll.generate') }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="row hidden">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="overtime_rate" class="form-label hidden">Overtime Rate (₱)</label>
                                <input type="number" class="form-control" id="overtime_rate" name="overtime_rate" step="0.01" min="0" required value="0" hidden>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="alert alert-info">
                                ...
                                <strong>Automatic Deductions:</strong> Deductions will be calculated automatically based on each employee's configured deduction settings (SSS, PhilHealth, Pag-IBIG, Tax, etc.). Please ensure employees have their deduction settings properly configured before generating payroll.
                            </div>
                        </div>
                    </div>
                    <div class="mb-3">
                            ...
                            @foreach($employees as $employee)
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label" for="employee_{{ $employee->id }}">
                                        {{ $employee->name }}
                                        @php
                                            $deductionCount = \App\Models\EmployeeDeduction::where('employee_id', $employee->id)->enabled()->count();
                                        @endphp
                                        @if($deductionCount > 0)
                                            <small class="text-success">({{ $deductionCount }} deductions configured)</small>
                                        @else
                                            <small class="text-warning">(No deductions configured)</small>
                                        @endif
                                    </label>
                                </div>
                            @endforeach
